...
data "azurerm_storage_account" "terrastate123" {
  name                = "terrastate123"
  resource_group_name = "state"
}
#create container for remote state in azure storage account
data "azurerm_storage_container" "tfstate" {
  name                 = "tfstate"
  storage_account_name = data.azurerm_storage_account.terrastate123.name
